...
{
    class StringStep : DialogueStepBase
    {
        ...
        public override async Task<bool> ProcessStep(DiscordClient client, DiscordChannel channel, DiscordUser user)
        {
            ...
            while (true)
            {
                ...
                if (_minLength.HasValue)
                {
                    ...
                    {
                        await TryAgain(channel, $"Your input is {_minLength.Value - messageResult.Result.Content.Length} too short").ConfigureAwait(false);
                        continue;
                    }
                }
                if (_maxLength.HasValue)
                {
                    ...
                    {
                        await TryAgain(channel, $"Your input is {messageResult.Result.Content.Length - _maxLength.Value} too long").ConfigureAwait(false);
                        continue;
                    }
                }
